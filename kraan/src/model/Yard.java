    ...
    public boolean addItem(Item i) {
        ...
        for (int j = 0; j < slotList.size(); j++) {
            ...
            if (temp.getItem() == null) {
                ...
                double tijdBewegenNaarIngang = (Math.abs(inputSlot.getCenterX()) + x) * gantry.getXSpeed() + (Math.abs(inputSlot.getCenterY() + y) * gantry.getYSpeed());
                int pickUp = 2*pickUpPlaceDuration;
                double tijdBewegenNaarPlaats = (Math.abs(inputSlot.getCenterX()) + temp.getCenterX()) * gantry.getXSpeed() + (Math.abs(inputSlot.getCenterY() + temp.getCenterY()) * gantry.getYSpeed());
    ...
    public boolean maakVrij(Slot s) {
        ...
        if (s.getZ() < height) {
            ...
            if (Math.floor(s.getXMin() / 10) != s.getXMin() / 10) { // staggered - WERKT NOG NIET HEHEXD
                ...
                if (debug)
                    System.out.print("DEBUG - yCoords: " + yCoords + " xCoords: " + xCoords + " -> " + ((xCoords - 1) + (yCoords * length)) + " Z level: " + (s.getZ() + 1) + " || ");
